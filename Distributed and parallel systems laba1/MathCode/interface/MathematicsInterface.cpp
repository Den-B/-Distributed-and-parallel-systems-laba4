...
void MathematicsInterface::displayMatrixFile(Matrix my_Matrix, string operation, int place_on_one_number,string fileName)
{
	...
	for (int i = 0; i < my_Matrix.getNumberOfRows(); i++) {
		output << setw(place_on_one_number) << i << "  ";
		for (int k = 0; k < my_Matrix.getNumberOfColumns(); k++) {
			output << setw(place_on_one_number) << dynamic_cast<FloatDataType*>(my_Matrix.getValueAt(i, k))->getValue() << " ";
		}
		output << endl;
	}
	output.close();
	...
	while (!isEndOfGlobal) {
		...
		switch (globalOption) {
			...
			case '2':
				...
				fixChangesInFileCons("MatrixMultiThread.txt", "Время выполнения: " + to_string(endclock - beginclock) + ".");
				...
				fixChangesInFileCons("MatrixOneThread.txt", "Время выполнения: " + to_string(endclock - beginclock) + ".");
